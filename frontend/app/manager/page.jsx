...
import { getManagerOverview } from '../../lib/api';
...
function PieTooltip({ active, payload }) {
  ...
  const { payload: inner } = payload[0];
  return (
    <div className="rounded-xl bg-white px-3 py-2 shadow-lg ring-1 ring-slate-200 text-xs">
      <p className="font-semibold text-slate-700">{payload[0].name}</p>
      <p className="text-slate-500">{inner.isTime ? fmtHours(payload[0].value) : `${payload[0].value} break${payload[0].value !== 1 ? 's' : ''}`}</p>
    </div>
  );
}
...
export default function ManagerPage() {
  ...
  useEffect(() => {
    ...
    if (!['manager', 'admin'].includes(user?.role)) { router.replace('/dashboard'); return; }
  }, [router]);
  ...
  return (
    <div className="flex min-h-full flex-col bg-slate-50">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-6 sm:px-6">
        ...
        {!loading && !department && (
          <div className="rounded-2xl bg-amber-50 p-6 ring-1 ring-amber-200 text-center">
            <p className="text-sm font-medium text-amber-800">No department assigned to you yet.</p>
            <p className="mt-1 text-xs text-amber-600">Ask an admin to assign you as a department manager.</p>
          </div>
        )}
              ...
              <div className="mb-6 grid grid-cols-1 gap-5 lg:grid-cols-3">
                        ...
                        <PieChart>
                          ...
                          <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }}
                            formatter={(value, entry) => (
                              <span style={{ color: '#64748b' }}>{entry.payload.name} ({fmtHours(entry.payload.value)})</span>
                            )}
                          />
                        </PieChart>
                      </ResponsiveContainer>
                ...
                {breakPieData.length > 0 && (
                  <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
                    ...
                    <div className="h-48">
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={breakPieData} cx="50%" cy="50%" outerRadius={72} paddingAngle={2} dataKey="value">
                            {breakPieData.map((d, i) => <Cell key={i} fill={d.color} />)}
                          </Pie>
                          <RechartTooltip content={<PieTooltip />} />
                          <Legend iconType="circle" iconSize={8} wrapperStyle={{ fontSize: 11 }}
                            formatter={(value, entry) => (
                              <span style={{ color: '#64748b' }}>{entry.payload.name} ({entry.payload.value})</span>
                            )}
                          />
                        </PieChart>
                      </ResponsiveContainer>
                ...
                {topUsers.length > 0 && (
                  <div className={`rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 ${prodPieData.length === 0 || breakPieData.length === 0 ? 'lg:col-span-2' : ''}`}>
                    <h2 className="mb-1 text-sm font-semibold text-slate-700">Top Active Members</h2>
                ...
                {STATUS_FILTERS.map((f) => (
                  <button key={f.value} onClick={() => setFilter(f.value)}
                    className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${
                      statusFilter === f.value ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50'
                    }`}>
                    {f.label}
                  </button>
                ))}
                          ...
                          {u.todayTotal > 0 ? (
                            <div className="flex items-center justify-end gap-2">
                              <div className="h-1.5 w-16 rounded-full bg-slate-100">
                                <div className="h-1.5 rounded-full bg-emerald-400" style={{ width: `${u.todayEfficiency}%` }} />
                              </div>
            ...
            {!loading && breakCategories.length > 0 && (
              <div className="mt-6 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div className="border-b border-slate-100 bg-slate-50 px-5 py-3">
                  ...
                  <p className="text-xs text-slate-400">Aggregated by category across department</p>
                </div>
                <table className="w-full text-sm">
                  <thead>
